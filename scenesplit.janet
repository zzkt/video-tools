...
(defn find-videos [input]
  (case (os/stat input :mode)
    :file (if (video-file? input)
            [input]
            (do (printf "Warning: %s not a supported video" input) []))
    :directory (map (fn [f] (string input "/" f))
                    (filter video-file? (os/dir input)))
    (do (printf "Warning: %s does not exist" input) [])))


# Filename and path sanitisation
(defn lavfi-escape-filter [s]
  (reduce (fn [acc c] (string/replace-all c (string "\\" c) acc))
          s ["," ";" "[" "]"]))

(defn tmp []
  (string (or (os/getenv "TMPDIR") "/tmp") "/ffprobe_" (math/random)))
...
(defn lavfi-safe [video t]
  (if (some (fn [c] (string/find (string/from-bytes c) video)) ":'\"?;&")
    (let [ln (string t ".ln.mp4")
      ...
      {:path ln :symlink true})
    {:path video :symlink false}))


# ffmpeg wrapping
(defn scene-timestamps [video threshold]
  (def t (tmp))
  (def lavfi-info (lavfi-safe video t))
  (def lavfi-video (lavfi-escape-filter (lavfi-info :path)))
  (shell "sh" "-c"
         (string "ffprobe -v " loglevel
                 " -show_entries frame=pts_time -of csv=p=0 "
                 "-f lavfi \"movie=" lavfi-video
                 ",select=gt(scene\\," threshold ")\""
                 " > " (sh/escape t)))
  (when (lavfi-info :symlink) (os/rm (string t ".ln.mp4")))
  (def result (try (slurp t) ([_] "")))
  (os/rm t)
  (filter |(not= $ "")
...
(defn extract-thumb [video ts idx out]
  ...
  (shell "sh" "-c"
         (string "ffmpeg -v " loglevel
                 " -y -ss " ss " -i " (sh/escape video)
                 " -vframes 1 -q:v 2 " (sh/escape dst)))
  dst)
...
(defn process-video [video out-dir threshold cut ext]
  ...
  (def clips (when cut
    (map (fn [i]
           ...
           (file/flush stdout)
           (cut-clip video ts (when (< (+ i 1) n)
                                (- next-ts ts)) i out-dir ext))
         (range n))))
  (when clips (file/write stdout "\n"))
  (def result (map (fn [i]
         (def ts (or (get timestamps i) "0"))
         (file/write stdout
                     (string "\r  [" (+ i 1) "/" n "] Extracting scene_"
...
(defn write-html [videos path theme]
  ...
  (buffer/push b
    ...
    "<title>Video Scene index</title><style>"
    (if (= theme "light") css-light css-dark)
    "</style></head><body>")
    (each video videos
    (buffer/push b (string/format "<div class='video-section'><div class='video-title'>%s</div><div class='scenes-grid'>" (esc (video :name))))
    (each scene (video :scenes)
      (buffer/push b (string/format "<div class='scene-card'><img src='%s' alt='Scene %d'> <div class='scene-info'><span class='scene-num'>Scene %d</span><span class='scene-time'>%s</span>"
...
alongside the thumbnails (uses stream copy for speed).```
  ...
  (printf "Input:  %s\nOutput: %s\nThreshold: %g" input output threshold)
  (when cut (printf "Cut mode: ON (ext: %s)" ext))
  (ensure-dir output)
  (def videos (find-videos input))
  (when (empty? videos) (print "No video files found.") (break))
  (printf "Found %d video file(s)" (length videos))
  (def results @[])
  ...
  (each video videos
    ...
    (printf "Processing file [%d/%d]: %s" idx total name)
    (def scenes (process-video video dir threshold cut ext))
    (printf "  %d scene(s) detected" (length scenes))
    (each s scenes
      (put s :rel-path (string name "/" (last (string/split "/" (s :image)))))
      (when (s :clip)
        (put s :rel-clip (string name "/" (last (string/split "/" (s :clip)))))))
    (array/push results {:name name :scenes scenes}))
  (sort results (fn [a b] (< (a :name) (b :name))))
  (write-html results
              (string output "/scenes.html")
              (if light "light" "dark"))
